...
- **Next sub-chunk: 1a.5** — variants (operator UX for SKUs, prices, stock; the data layer is partially in place via `product_options` / `product_option_values` / `product_variants` from Phase 0).
- **Then:** 1a.6 (bilingual content polish + missing-translation badge), 1a.7 (image upload pipeline).

**Tests at end of 1a.4.3:** 886 vitest across 107 files + 730 Playwright passed / 110 skipped on the full matrix (iphone-14-{en,ar}, pixel-7-{en,ar}, desktop-chromium-{en,ar}). lint, typecheck, e2e-coverage, role-invariants all clean.

**Tracked follow-up from 1a.4.3:** the chip-removal focus assertion in `tests/e2e/admin/products/product-categories-picker.spec.ts:213` ("chip × button removes the category from the local set; Save persists") deterministically fails on the mobile projects and intermittently on desktop after the parallel-test load increased — the `requestAnimationFrame` + React reconciliation race in `edit-product-form.tsx:onRemoveChip` is the underlying cause. The test was passing at 1a.4.2 close. Code in `edit-product-form.tsx` was not modified by 1a.4.3. Either tighten the focus handler (await reconciliation via `flushSync` or a state-driven post-render effect rather than a single `requestAnimationFrame` tick) or quarantine the test with a tracked TODO. Pick this up before 1a.5 begins so it doesn't ghost the next chunk's run.
