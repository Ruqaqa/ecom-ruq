...
**Phase 0 closed 2026-04-23.** Full chunk-by-chunk history (chunks 1–11, scope-split decision, deliverables, test counts) lives in `git log` and `docs/runbooks/`. Read those for specifics — do not assume from memory.

**Phase 1a is in progress (started 2026-04-25).** Owner deferred Launch infrastructure to the top of Phase 1b ("catalog tooling first; no public storefront yet"). It stays queued until the storefront is ready to go live.

Sub-chunks shipped (one line per chunk; commit log + runbooks carry the detail):

- **1a.1** Admin product list (2026-04-24).
- **1a.2** Edit a product (2026-04-25, +3 same-day follow-ups).
- **1a.3** Soft-delete + restore + owner-only purge for products (2026-04-25). 30-day recovery; MCP `delete_product` / `restore_product` / `hard_delete_expired_products` all `confirm: true`.
- **1a.4.1** Categories foundation — services + tRPC + MCP, no UI (2026-04-26). Many-to-many products↔categories, depth ≤ 3, single Latin slug per category. Same-day defense-in-depth migration `0010` adds composite same-tenant FKs.
- **1a.4.2** Categories admin + product-edit multi-pick (2026-04-27). Shared `CategoryPickerSheet`; `setProductCategories` set-replace; cap 32/product. Bundled fix: e2e global setup now sweeps stale `e2e-` rows from `products` and `categories` before each run.
- **1a.4.3** Category soft-delete + restore + owner-only purge sweeper (2026-05-01). Cascade-soft-delete on parent removal; single-row restore only with four guards. Sweeper does unbounded subtree walk with `seen` set (defense-in-depth from security review). Same-day follow-up: chip-removal focus race fix on product edit (replaced `requestAnimationFrame` trampoline with `flushSync` in `edit-product-form.tsx:onRemoveChip`; was a flake on mobile WebKit under parallel test load).
- **1a.5.1** Variants foundation — services + tRPC + MCP, no UI (2026-05-01). 3 option types max, 100 variants max, set-replace semantics, single-variant default mode. Migration `0011` adds composite same-tenant FKs + JSONB-array-≤3 CHECK. SKU collisions → opaque `sku_taken` (no SKU echo, audit row absent on failing path).
- **1a.5.2** Variants admin UX core — Options panel + Variants list (2026-05-01). Composite read `getProductWithVariants`; four-leg save chain (`products.update` → `setCategories` → `setOptions` → `setVariants`) threading OCC token forward. New shared primitives: `<BottomSheet>`, `<InlineNumberField>`. Mobile-first vertical list-of-cards at 360px (not a horizontal-scroll table).
- **1a.5.3** Variants admin UX polish (2026-05-02). Six surfaces on product edit: bulk-apply, per-row remove, remove-option-type cascade dialog, cap-hit pre-save advisory, opaque SKU-collision banner with no row decoration / no `aria-invalid` / no focus shift (closes the existence-leak channel — Playwright scenario 9 byte-equality enforces it), State-C live transitions both directions. Service-side: `setProductOptions` cascades on omission within its single-tx + per-product advisory lock; audit snapshot extends with `cascadedVariantIds`. No new tRPC procedures, no new MCP tools — all UX flows through existing set-replace surfaces. Same-day follow-ups (2026-05-02): variant price input now commits-on-blur via a `<VariantPriceField>` wrapper (per-keystroke `priceMinor` round-trip was eating cents — typing "12.34" left "1.00"); State-C expand banner only fires when the flat form has typed-in data and copy moved to future-tense.
- **1a.7.1** Image pipeline foundation (2026-05-02). Storage adapter (swappable from day one — local disk in dev, BunnyCDN in prod), Sharp-based processing pipeline with sizing rules + EXIF strip + dedupe via fingerprint + replace-in-place, services + tRPC + MCP. Per-product library + per-variant cover; gallery deferred. Video upload path noted but out of scope.
- **1a.7.2** Admin photo screens + same-day follow-ups (2026-05-03). Photos section on product edit page: upload, set-cover, edit alt, replace, remove, drag-reorder, drag-to-upload from OS file manager (desktop-only). Same-day follow-ups: OCC token threading fix between photo mutations and the form save chain (photos-section lifts fresh `productUpdatedAt` back to the parent on every mutation); drag-reorder optimistic update with rollback on server rejection; drag-to-upload zone with file/non-file payload discrimination. Bundled simplify pass: O(n) reorder memo (`Set.has` instead of `.includes`), hoisted `isStaleWriteCode` to module scope, removed unused `readFileBytes` helper, replaced racy `waitForTimeout` non-appearance assertions with `.not.toBeVisible({ timeout })`. Drag-handle reliability fix (same day): pinned a stable `id` on `DndContext` to eliminate dnd-kit's module-counter hydration mismatch (server rendered `DndDescribedBy-1`, client `-0`, React aborted hydration of the subtree and left the handle in its SSR-disabled state); switched `PointerSensor` from `delay: 250, tolerance: 8` (press-hold) to `distance: 5` because the dedicated handle button already signals intent — finger jitter inside the delay window had been silently cancelling activation, reading to the operator as "sometimes the handle is dead."

**Tests at end of 1a.5.3 (post-follow-ups):** 984 vitest across 117 files + 857 Playwright passed / 115 skipped on the full matrix (iphone-14-{en,ar}, pixel-7-{en,ar}, desktop-chromium-{en,ar}). Lint, typecheck, e2e-coverage, role-invariants all clean. Counts have **not** been re-taken since 1a.7.x landed — re-run before declaring 1a.7 closed.

**Next sub-chunk:** owner discretion. 1a.7 image work is in for the must-have product-library + per-variant-cover scope; gallery and video remain deferred. 1a.6 (bilingual content polish + missing-translation badge) was **deferred on 2026-05-02** to a post-must-have polish pass — the JSONB `{ en, ar }` input pair already ships on products/variants/categories from 1a.2 onward, so the deferred scope is only the badge UI surface and (if still wanted) a "show only items missing Arabic" filter. Owner's reasoning: pre-launch, solo operator, small catalog → the badge mostly tells the owner what they already know; better revisited closer to launch when the catalog is large enough that the badge actually saves time. Picks up after Phase 1a images, Phase 1b storefront, and checkout/payments are in.
