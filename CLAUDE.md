...
- **Next sub-chunk: 1a.4.3** — soft-delete + restore + owner-only hard-purge sweeper mirroring 1a.3 (`delete_category` / `restore_category` / `hard_delete_expired_categories`, all `confirm: true`). Cascades soft-delete to descendants in the same transaction; refuses to restore a child whose parent is still soft-deleted.
- **Then:** 1a.5 (variants), 1a.6 (bilingual content polish + missing-translation badge), 1a.7 (image upload pipeline).

**Tests at end of 1a.4.2:** 786 vitest across 98 files + 619 Playwright passed / 65 skipped (full matrix: iphone-14-{en,ar}, pixel-7-{en,ar}, desktop-chromium-{en,ar}). lint, typecheck, e2e-coverage, role-invariants all clean.
