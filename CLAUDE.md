...
- **1a.5 — Variants.** Scoped 2026-05-01. Decisions locked per prd §3.3: option-driven model matching Shopify/Amazon, per-product option names with no global registry, at most 3 option types per product, at most 100 variants per product, single-variant default mode for products without variations, set-replace semantics for both options and variants (no patch), no per-variant photos until 1a.7 lands the image pipeline, per-tenant specifications + catalogue filters as a separate later chunk, customer-facing variant picker as part of Phase 1b's product detail page. Three sub-chunks land in order:
  ...
  - **1a.5.3 — Admin UX polish (cascade-on-remove-option + bulk + inline errors + State-C live):** landed 2026-05-02. Service-layer side: the 1a.5.1 transitional refusal `option_remove_not_supported_yet` is gone; `setProductOptions` now performs cascade-on-omission inside its existing single-tx + per-product advisory lock — when the operator's options-set omits an existing option type (or one of its values), every variant row that referenced any of the doomed values is hard-deleted in the same transaction. The doomed-variant SELECT keeps every value-id flowing through prepared-statement parameter binding (no SQL-string interpolation). Audit snapshot extends `OptionsAuditSnapshot.after` with `cascadedVariantIds: string[]` (sorted, ids only — no SKU, no localized name/value text — included in the hash payload, ≤100 ids per call so the snapshot stays well under the 64KB cap). The wire envelope returns `cascadedVariantIds` so the UI can prune local row state synchronously after the call lands. **No new tRPC procedures, no new MCP tools** — bulk-apply, per-row removal, and option-type cascade-removal all flow through the existing `setProductOptions` / `setProductVariants` set-replace surface (architectural commitment from 1a.5.1 preserved). The `set_product_options` MCP tool description is rewritten verbatim per the security spec to declare the cascade-on-omission contract for autonomous agents; `set_product_variants` description unchanged. No `confirm: true` on either tool (consistency with `set_product_categories`; the description carries the contract). Cross-tenant adversarial Level B coverage extends with three new probes (cascade-vs-no-cascade envelope byte-equality, cascade audit-row-presence + audit_payloads join asserting `cascadedVariantIds` actually persists into the append-only chain, variant-row removal happy path). **No migration required** — the 1a.5.1 composite-FK + value-id CHECK + advisory-lock infrastructure already covers this chunk's on-disk semantics. Admin UI side: six surfaces shipped on the product edit page — (a) `<RowSelectionToolbar>` sticky bottom bar (selection count + Select-all-visible + Apply CTA) feeding a new `BulkApplySheet` (drops in the 1a.5.2 `<BottomSheet>` primitive, reuses `<InlineNumberField>` for the apply-price + apply-stock inputs, client-side state mutation that the existing save chain commits); (b) per-row kebab → inline red confirm strip (the row's `key` adds to `removedKeys`; `setVariants` payload omits the key on save, triggering the existing hard-delete-on-diff-removal contract); (c) the previously-disabled `option-remove-cta` is wired live, opening a new `<RemoveOptionDialog>` mirroring the 1a.4.3 cascade pattern with a live ICU-plural count of variant rows that will be removed and an explicit collapse-preview line when removing the last option type; (d) cap-hit pre-save advisory — amber banner under the cap counter when `options × values` would generate more than `MAX_VARIANTS_PER_PRODUCT = 100`, **convenience only** (server is the trust boundary via Zod max + migration-0011 column CHECK), bypassed warnings surface at `variants-top-error` with the bound copy and a tightened branch in the save-error router that distinguishes the cap-exceeded `too_big` Zod issue from generic validation failures so the operator gets the right message; (e) duplicate-SKU two-path handling: client-side pre-check pins both rows when two non-empty SKUs collide in the operator's own draft (both values local — no leak); server-side `sku_taken` surfaces as a single opaque `variants-top-error` banner above the variants list with **no row decoration, no `aria-invalid`, no auto-scroll, no focus shift** (security-bound to close the existence-leak channel that would otherwise let a compromised staff PAT enumerate sibling-tenant SKUs by binary-searching the response — Playwright scenario 9 enforces byte-equality on DOM subtree innerHTML + focus fingerprint + scrollY across two runs with swapped typing order); (f) State-C live transition wired live in both directions via a `<TransitionNotice>` dismissible-banner primitive — multi → single (last option removed via cascade dialog) collapses the grid back to flat single-variant form on the spot with the first variant's typed SKU/price/stock preserved as the default-row's seed values via the new `buildVariantRows({ transitionMergePolicy: "preserve-first-touched" })` extension; single → multi (first option type added) flips on a `useFlatFormSeed` flag so the cartesian generator pre-fills the first generated row from the flat form's typed data on the first option-value entry. Locale catalogs added `admin.products.edit.variants.bulkApply.*` / `removeOption.*` / `capWarning.*` / `collapseNotice.*` / `expandNotice.*` in both EN and AR with full ICU plural keysets (zero/one/two/few/many/other) for AR per CLDR.

  - **1a.5.3 same-day follow-ups (2026-05-02): variant price input + smarter expand banner.** Two small bugs surfaced during owner browser-testing of the chunk close. First: the variant price field's per-keystroke round-trip through `priceMinor` (cents) reformatted "3" into "3.00" between keystrokes, parking the cursor after the decimals so subsequent characters landed in the .00 slot and rounded silently to the same cents value — typing "12.34" left the field showing "1.00". Playwright's `fill()` set the entire value at once and never exercised the per-keystroke path so 1a.5.3's polish spec missed it. Fix: a `<VariantPriceField>` wrapper inside `variants-list.tsx` holds a draft text string while focused, only commits the parsed cents on blur, and resyncs from the parent's `priceMinor` when not focused (so bulk-apply pushes still reach the field). `<InlineNumberField>` gained optional `onFocus` / `onBlur` props to support the wrapper. Both call sites (single-variant flat form + multi-variant cards) use the wrapper; stock is unaffected (integer round-trip is stable). Regression: a new Playwright scenario types "12.34" character-by-character via `pressSequentially(..., { delay: 30 })` on both the flat form and a multi-variant row card, asserts the visible value matches mid-typing AND after blur re-format, then saves and re-reads `price_minor` from the database — runs in both locales on the full mobile + desktop matrix and verified to FAIL when the wrapper is reverted. Second: the State-C single → multi expand banner ("We saved your SKU, price, and stock...") fired the moment the operator tapped Add an option type, even on a brand-new product where the flat form had nothing typed in — promising preservation that had nothing to land on, and reading in past tense before the first variant row even existed. Fix: the banner's trigger now reads the latest flat row via a `variantRowsRef` (mirrors the existing `optionsStateRef` pattern) and only fires when the flat row has typed-in SKU, price, or stock at the moment of expansion; the seed flag `useFlatFormSeed` moves under the same gate. Copy rewritten to two clear future-tense clauses ("We saved your SKU, price, and stock. They'll appear on the first variant row after you add option values.") with the same shape in AR. Regression: a new Playwright scenario asserts the banner does NOT appear on the empty-flat path and DOES appear after typing into the flat form, in both locales on the full matrix.
