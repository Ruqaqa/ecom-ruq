...
**Phase 0 is complete.** Chunks 1–7, 9, 10, and 11 have all landed to `main` (chunk 11 closed 2026-04-23).

**Phase 1a is in progress (started 2026-04-25).** The owner deferred the Launch-infrastructure pass at the top of Phase 1b — the rationale was "no server is needed yet; the catalog tooling is more useful first." Phase 1a chunks land first; Launch infrastructure stays queued at the top of Phase 1b for whenever the public storefront is ready to go live. Current state on `main`:

- **1a.1 — Admin product list** (landed `1ef9bc6`, 2026-04-24): RSC at `/{locale}/admin/products` with a card list on mobile and a table on desktop (status pill, last-updated date, slug). `listProducts` service + tRPC `products.list` query + read-only MCP `list_products` tool. Cursor-paginated forward-only on `(updated_at DESC, id DESC)`, opaque base64url cursor; garbage cursors silently fall back to the first page. Tier-B `cost_price_minor` is owner-only (gated at SELECT column list AND output schema). New `pickLocalizedName` helper at `src/lib/i18n/`. Backed by 60 vitest + 36 Playwright runs.
- **1a.2 — Edit a product** (landed `3a8f846`, 2026-04-25 + three follow-ups: `06ab8c8` riyal-display on form, `2d7e84a` MCP boundary speaks SAR, `43108e6` SlugTakenError + shared slug schema + dedup'd pg helpers): RSC at `/{locale}/admin/products/[id]` + `getProduct` service + `products.get` query + `products.update` mutation + MCP `update_product` tool with `auditMode:"mutation"`. Sparse-update with `key in input` semantics: absent leaves alone, explicit-null clears (owner-only). Optimistic concurrency via required `expectedUpdatedAt` produces a new closed-set audit code `stale_write` distinct from slug-collision `conflict`. Edit form copy-and-adapts the create form (deferred extraction to 1a.7); sticky bottom action bar, in-app dirty-state confirm + `beforeunload`, slug-change warning, slug input `dir="ltr"` inside RTL. **Form + MCP boundary speak SAR (riyals)**; service layer keeps storing halalas (exact integer math). New domain errors `StaleWriteError` and `SlugTakenError` (services stay transport-neutral; tRPC procedures translate to `TRPCError CONFLICT`). Audit `before`/`after` carry full Tier-B for staff edits via `auditPayloads`/`auditOverride` seam on both transports. Carry-overs: `createProduct` slug-collision now throws `SlugTakenError` (was previously bubbling pg 23505 as a 500). Shared `slugSchema` exported from `@/lib/product-slug`. Shared pg-cause-peeler at `src/server/db/pg-errors.ts` consumed by both the audit mapper and the products' helpers.

**Tests at end of 1a.2 cleanup (`43108e6`):** 584 vitest across 77 files + 406 Playwright passed / 20 skipped (both up substantially from chunk 11 close). lint, typecheck, e2e-coverage, role-invariants all clean.

**Next chunk:** 1a.3 — soft-delete a product (recovery window, owner+staff, MCP `delete_product` with `confirm: true` flag). After that: 1a.4 (categories), 1a.5 (variants), 1a.6 (bilingual content polish + missing-translation badge system-wide), 1a.7 (image upload pipeline). Launch infrastructure remains queued at the top of Phase 1b for when the storefront is ready to go live.

The deployment-target-dependent items (hosted CI pipeline, Coolify deploy webhook, Sentry DSN, Lighthouse enforcement, CDN, backups, uptime monitoring) were moved out of Phase 0 by the **2026-04-23 scope-split decision** and now land as part of the **Launch infrastructure** block at the top of Phase 1b in `prd.md`.
...
Tests at chunk 10 close: **450 vitest across 65 files + 298 Playwright passed / 20 skipped** (+34 vitest, 0 Playwright delta — chunk 10 adds safety guards but does not alter user-facing behavior). All green.

Tests at chunk 11 close: **451 vitest across 66 files + 298 Playwright passed / 20 skipped** (+1 vitest, 0 Playwright delta — chunk 11 fixes the silent magic-link audit-row loss but does not alter user-facing behavior). All green. **Phase 0 complete.**

**Scope-split context (2026-04-23):** The original "chunk 9" was a hosted GitHub Actions CI pipeline + Coolify deploy webhook + Sentry wiring + Lighthouse enforcement — a single large chunk. On 2026-04-23 the owner and I deliberately split it because its deployment-target-dependent pieces have no Hetzner VM / Coolify instance to point at today and no second-developer pushes to guard. Those pieces (Hetzner VM, Coolify, GitHub Actions workflow, Coolify deploy webhook, Lighthouse CI enforcement, Sentry DSN, CDN, backups, uptime monitoring) moved to a new **Launch infrastructure** block at the top of Phase 1b in `prd.md` — they land as one coordinated pass before the public URL goes live. Only the pieces that were load-bearing today stayed inside Phase 0, plus the pre-existing magic-link audit bug as chunk 11.
...
**Chunk 10 — Boot-time + request-time production-safety guards — LANDED.** New module `src/server/boot/production-guards.ts` exports `isRealProduction()`, `assertDangerousEnvFlagsUnset()`, `assertBetterAuthDbRoleSafe()`, `assertProxyHeaderPresent(headers)`, `runBootGuards()`, `ProductionGuardError`. Boot guards fire from a new `src/instrumentation.ts` (Next.js 15 one-shot boot hook). Request-time proxy-header guard is called from `src/middleware.ts` (page routes → 503 `proxy_header_missing`) AND from the top of `enforceAuthRateLimit` in `src/server/auth/rate-limit-auth-hook.ts` (covers `/api/auth/*`, which the middleware matcher excludes — converts to `APIError("SERVICE_UNAVAILABLE", { code: "PROXY_HEADER_MISSING" })`). **Key design decision:** "real production" = `NODE_ENV === "production"` AND `APP_ENV NOT IN {"e2e", "seed"}` — the APP_ENV carve-out keeps the Playwright harness (which runs under `pnpm build && pnpm start`, forcing NODE_ENV=production per chunk 8) from refusing to boot. This matches the existing codebase convention (`rate-limit-auth-hook.ts`, `audit-hooks.ts`). The spec originally said "NODE_ENV=production" but that would refuse to boot under the e2e harness; updated wording reflects the real signal. CI env-lint in Phase 1b Launch infrastructure is the backstop that catches `APP_ENV=e2e` in a real prod deploy. Dangerous-flag list deliberately omits `APP_ENV=e2e|seed` (self-excluding by definition) and checks `E2E_AUTH_RATE_LIMIT_DISABLED=1` + `MCP_RUN_SQL_ENABLED=1`. Better Auth DB URL now prefers `DATABASE_URL_BA` → `DATABASE_URL_APP` → `DATABASE_URL` (`src/server/auth/auth-server.ts`); `.env.example` documents the new variable. Security pass folded in: DB role guard inspects BOTH URL userinfo AND `?user=` query param (postgres-js honors the latter and it wins at connect time), NFKC-normalizes to fold Unicode compatibility variants (e.g. fullwidth underscore), and wraps `decodeURIComponent` in try/catch so malformed percent-encoding surfaces as a `ba_db_url_unparseable` guard error. +34 vitest tests in `tests/unit/boot/production-guards.test.ts`. Known deferred items (documented in the security review, scheduled for Phase 1b): TRUSTED_PROXY_CIDR socket-peer check defending against a misconfigured front-end, `runtime = "edge"` lint ban, and tRPC/MCP route-handler coverage of `assertProxyHeaderPresent`.

**Chunk 11 — Magic-link audit_payloads PK collision fix — LANDED.** The bug turned out to be deterministic, not race-dependent as originally suspected: `sessionCreateAfter` writes two audit rows on every magic-link consume (`auth.session.create` and `auth.magic-link.consume`) sharing one correlationId by design so operators can join them. Both rows carry an `after` payload, and the old `audit_payloads` PK `(correlation_id, kind)` rejected the second write — the entire second transaction rolled back via the swallow-in-catch path in `writeAuditInOwnTx`, so BOTH the headline `auth.magic-link.consume` row AND its detail row were silently lost on every magic-link sign-in (worse than the original race framing suggested). **Fix:** new migration `0008_audit_payloads_widen_pk.sql` swaps the composite PK for a surrogate `id uuid DEFAULT gen_random_uuid()` PK and adds a non-unique lookup index on `(correlation_id, kind)` to preserve operator-forensic joins; `src/server/db/schema/audit.ts` updated to match. Insert path in `src/server/audit/write.ts` was untouched — the new `id` column defaults at the DB. RLS policy in `0002_rls_policies.sql` is keyed on `tenant_id` only and needed no change. Regression test at `tests/unit/audit/magic-link-consume-payloads.test.ts` drives the two-write-same-correlationId scenario and asserts both detail rows land (+1 vitest file, +1 test). The shared-correlationId design property is preserved.
...
**Note for deferred scope** (see `prd.md` §4 "Deferred"): `pgvector`, Voyage AI embeddings, and the customer-facing AI bot are **post-revenue**. Do not pull them into Phase 1 work. Owner admin chat ("Chat with your store", Phase 4) is NOT deferred — it uses MCP tools + `run_sql_readonly`, no embeddings.

**Resume guidance.** Running `pnpm install` and `pnpm services:up` brings the local stack up; `pnpm dev` serves at `http://localhost:5001`. All commands in the table below work today. Phase 0 is closed; the next block of work is the **Launch infrastructure** pass at the top of Phase 1b (see `prd.md`). The memory index at `~/.claude/projects/-Users-bassel-development-ecom-ruq/memory/MEMORY.md` has project-specific context (ports, deferred scope, user background, team workflow lessons) — read it before starting a chunk.
